...
- 启动流程 / 启动耗时：`docs/dev-harness/startup/rules.md`
...
适用对象：所有 AI 编码助手（Claude Code、Codex、其它接入本仓库的工具）。

- Plan 模式下完成"读代码 → 拆步骤"的工作项拆分后，**推荐用 sub-agent 执行**每个可独立验证的步骤，主 agent 只负责任务分发、阶段汇总、最终验证。
- 触发条件：单个工作项满足以下任一条件，应派给 sub-agent：
  - 涉及多个文件的批量改动（≥ 3 文件），且步骤之间无强顺序耦合；
  - 步骤本身是一段封闭工作（如"补一个模块的日志埋点 + 单测"），不依赖主 agent 的对话上下文；
  - 步骤需要大量探索性 grep / read，但只把结论汇总给主 agent 即可继续推进。
- 例外（必须主 agent 亲自做）：
  - 需要逐条用户审批的设计决策、API 选型；
  - 跨步骤的关键状态变更（合并、发布、删除分支）；
  - 单文件 3 行以内的微调。
- 派发 sub-agent 时遵守：
  - 按任务复杂度选 haiku / sonnet / opus，不全部走默认（详见个人记忆 `feedback_subagent_model_selection`）；
  - prompt 中必须自带上下文（路径、相关函数、约束、验收点），不能假设 sub-agent 看过主对话；
  - 多个独立 sub-agent 必须并行派发（单条消息多 tool call）。
- 主 agent 拿到 sub-agent 结果后必须做 trust-but-verify：抽样读关键 diff、跑相关单测，不能只复述 sub-agent 的总结。
...
排查 release 构建的线上崩溃 / ANR 时，类名都会是 `wh2.f`、`gw.g` 这种 R8 混淆名。禁止凭混淆名直接猜测源码位置，必须先按 [docs/dev-harness/release-mapping.md](docs/dev-harness/release-mapping.md) 下载对应版本的 `mapping-vX.Y.Z.zip` 并反查；版本号必须与崩溃版本严格一致。

### 启动流程与耗时守门

修改 `MusicFreeApplication`、`MainActivity`、SplashScreen、启动后台 coordinator、冷启动恢复或启动耗时日志前，必须读取并遵守 [docs/dev-harness/startup/rules.md](docs/dev-harness/startup/rules.md)。

- 启动日志必须区分 `cold_process_start`、`warm_activity_recreate`、`activity_resume_existing`。
- `StartupBackupRestore.applyIfPending()` 必须保持在 `attachBaseContext()`，不得因为埋点或重构推迟到 Room / DataStore 打开之后。
- `installSplashScreen()` 必须保持在 `super.onCreate(savedInstanceState)` 之前。
- 启动路径新增主线程同步工作必须保持 O(1) 且提供耗时证据；不得在首屏路径同步等待插件、网络、Runtime restore、更新检查或大 payload 读取。
...
- 日志与缓存复用排查见 [docs/dev-harness/player/cache-and-logs.md](docs/dev-harness/player/cache-and-logs.md)。
