 ...
 */
class JsLoadedPlugin(
    override val info: PluginInfo,
    private val engine: JsEngine,
    filePath: String,
    private val projector: MusicItemBridgeProjector,
    override val installSource: PluginInstallSource = PluginInstallSource(PluginInstallSourceType.LOCAL_FILE),
) : LoadedPlugin {

    // Mutable because PluginManager rewrites it after the staged-file → target-file
    // atomic move (see installWithStagedFile). Initialised from constructor arg.
    ...
    private suspend fun doGetMediaSource(musicItem: MusicItem, quality: String): MediaSourceResult? {
        injectGlobalMap("__musicItem", projector.project(musicItem))
        val result = engine.evaluate<Any?>(
    ...
    override suspend fun getMusicInfo(musicItem: MusicItem): MusicItem? {
        ...
        return executeApiCall(
            ...
            injectGlobalMap("__musicBase", projector.project(musicItem))
