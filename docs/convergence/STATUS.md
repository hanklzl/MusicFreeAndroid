...
| # | 差异项 | 粒度 | 综合分 | 来源 |
|---|--------|------|--------|------|
| 1 | 首页结构仍未完全对齐（Drawer、深色风格、歌单区视觉） | 细 | 5.5 | 迭代1/2/3/4/5验证 |
| 2 | `albumDetail/artistDetail` 页面仍缺失（API 已具备） | 粗 | 4.8 | 迭代5分析 |
| 3 | `musicDetail` 真实插件数据态与截图验收待补 | 中 | 4.2 | 迭代5验证 |
| 4 | TopListDetail 真实数据态未完成验收（缺稳定插件环境） | 中 | 3.8 | 迭代1/2/3/4/5验证 |
| 5 | PluginSheetDetail 真实数据态与播放链路待端上复验 | 中 | 3.6 | 迭代2/3/4/5验证 |
| 6 | History 列表态与回放链路（有数据）待补验收 | 中 | 3.5 | 迭代3/4/5验证 |
