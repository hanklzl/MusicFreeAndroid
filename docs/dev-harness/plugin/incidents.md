...
`:plugin/src/androidTest/.../PluginRuntimeIntegrationTest.kt` 旧版 4 个用例直连真实插件网络，CI 默认通道因网络抖动 flaky；类级 `@Ignore` 同时阻塞 3 个本地用例。

### 复发条件

新 `:plugin/src/androidTest/` 中出现真实第三方插件域名（登记在 PluginNetworkTestGateContractTest 的 liveHosts 中），但文件名不以 `NetworkIntegrationTest.kt` 结尾，或类内未 `Assume.assumeTrue` 引用 `pluginNetworkTests`。
