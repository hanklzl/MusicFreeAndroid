...
MUST NOT 把高价值运行态只放在 Compose 普通 `remember` 中。高价值运行态包括用户刚构建出来且恢复成本高的状态，例如详情页分页结果、播放会话、插件加载状态、下载队列、Route seed。

> 例外（2026-06-05，已评审）：**搜索会话状态**（query / 搜索结果 / 分页）已从进程级 RuntimeStore 降级为**页面生命周期**——`SearchSessionStore` 改为每个 `SearchViewModel` 一个实例（不再 `@Singleton`、不再进 RuntimeStore 集合）。配置变更经 ViewModel 保活仍保留；离开搜索页即随 ViewModel 一起清空；进程冷启动不再恢复（与 RN 行为一致）。理由：进程级单例导致离开搜索页后状态残留（搜索框不清空），且 `restore()` 覆盖 live 的 `searchablePlugins` 导致再次搜索卡在无结果。详见 [搜索状态页面生命周期降级](../../superpowers/specs/2026-06-05-search-session-page-lifecycle.md)。
...
> `SearchSessionStore` 曾在此列；自 2026-06-05 起降级为页面生命周期（见上方 `#rule-runtime-state-classification` 例外），不再作为进程级 RuntimeStore。
