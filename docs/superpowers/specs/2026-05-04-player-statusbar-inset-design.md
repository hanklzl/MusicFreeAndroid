...
> 直接执行：是（作为实现计划输入；具体代码改动需先生成实现计划）
...
- `feature/player-ui/src/main/java/com/zili/android/musicfreeandroid/feature/playerui/PlayerScreen.kt`
  - `PlayerScreen` 根节点是 `Box(modifier = Modifier.fillMaxSize())`。
  - Layer 1 是黑色背景层，使用 `fillMaxSize()`。
  - Layer 2 是封面模糊背景层，使用 `fillMaxSize()`。
  - Layer 3 是内容 `Column`，当前也从屏幕顶端开始。
  - `PlayerNavBar` 高度为 `rpx(150)`，内部包含返回按钮、标题、歌手/平台和分享按钮。
- `app/src/main/java/com/zili/android/musicfreeandroid/MainActivity.kt`
  - App 级 `Scaffold` 只处理横向和底部 safe inset。
  - `PlayerRoute` 可见时隐藏 MiniPlayer。
- `docs/ui-harness/screen-chrome-rules.md`
  - `PlayerRoute` / `PlayerScreen` 已登记为特殊 Chrome 页面。
  - 特殊 Chrome 页面不使用普通 AppBar，但必须自行负责状态栏背景和顶部 inset。

这些事实决定了修复位置应在 `PlayerScreen` 的内容层，而不是 `MainActivity`、背景层或 `PlayerNavBar` 内部。
...
状态栏 inset 应作用在内容层容器，而不是藏进 `PlayerNavBar`。原因是状态栏责任属于 `PlayerScreen` 这个特殊 Chrome 页面；如果把 inset 放进 `PlayerNavBar`，未来调整顶部区域时更容易遗漏或叠加 padding。
...
- 若有设备或模拟器，必须进入播放器页做运行态视觉验收；若没有设备，完成结论必须明确说明运行态视觉验收未执行。
