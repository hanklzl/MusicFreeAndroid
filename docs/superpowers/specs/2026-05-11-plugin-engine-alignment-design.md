...
| 方面 | RN | Android | 标记 |
|---|---|---|---|
| `mediaMeta` 跨次启动缓存 | MMKV `cache.MediaCache`，~800 项 LRU | 缺 | ★ |
| `MediaExtra.${pluginName}` | per-plugin 用户态：`downloaded` / `localPath` / `lyricOffset` / `associatedLrc` | 缺 | ★ |
| 歌词文件缓存 | `lrcCachePath/<nanoid>.lrc` + `localLrcPath/<platHash>/<idHash>.lrc` | 缺 | ★ |
| 卸载插件时连带清理 | `removeAllMediaExtra(pluginName)` | 缺 | ★ |
...
```
plugin.cacheControl ∈ {"cache", "no-cache", "no-store"}（默认 "cache"）

  "cache"     → 命中 cache 即返回；不命中 → fetch + 写 cache
  "no-cache"  → 网络可达 → fetch + 写 cache；网络不可达 → 读 cache（fallback）
  "no-store"  → 永不读写 cache（透传 fetch）
```

写 cache 时机：在 `getMediaSource` 成功返回非 null `url` 后由 `MediaMetaCacheGateway.put()` 落 LRU 内存 + Room。
...
1. **冷启动秒开**：装一份热门 RN 插件（候选：kuwo / kugou 社区版本）→ 搜歌 → 播放 → kill app → 重开 → 同首歌再播放，断言 `PluginMediaSourceService` 直接命中 `MediaMetaCache`（通过 Logan 日志 `plugin_get_media_source_cache_hit`）。
