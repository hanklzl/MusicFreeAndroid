...
`last_status` 枚举：`never_run` / `passed` / `diff_found` / `blocked_runtime` / `blocked_authoring` / `build_failed` / `rn_baseline_unstable`。
...
| **双边都崩** | 仍建 Issue（只针对 Android 侧），`kind=crash, severity=critical`，含 Android ANR/Tombstone 摘录；body "## 元数据" 段标 `rn_also_crashed=true` 并附 RN logcat 关键帧片段；REPORT 同时高亮 RN baseline 不稳，提示后续可能需要切换更稳定的 RN commit 重测 |
...
`verdict` 枚举：`passed` / `diff_found` / `rn_baseline_unstable` / `blocked_runtime`。

Android 崩溃判定优先于其它 verdict——只要本轮检测到 Android 进程崩 / ANR，`verdict` 一律记为 `diff_found`、`severity=critical`、`kind=crash`，与 RN 是否也崩无关；RN 同崩仅作为元数据写入 `diff.json.metadata.rn_also_crashed=true`，由 `file_issue.py` 在 Issue body 元数据段落里渲染。原"both_unstable"枚举项作废。
...
| Android 崩（无论 RN 是否崩） | `kind=crash, severity=critical` | 立即建 Issue，含 Android ANR/Tombstone 摘录；若 RN 同崩，body 元数据段落标 `rn_also_crashed=true` |
