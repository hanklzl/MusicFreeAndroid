...
> 备注：本文档记录修复设计，具体执行以随后生成的 implementation plan 为准。
> 最后校验：2026-05-03
...
同时将搜索页可搜索插件来源从直接 collect `pluginManager.plugins` 调整为 `pluginManager.getSearchablePlugins()`。这样搜索页天然复用插件管理层的启用/禁用和排序逻辑，避免搜索页自行过滤 raw plugin list 时绕过用户设置。
...
2. `SearchViewModel` collect `pluginManager.getSearchablePlugins()`。
...
第 4 点是为了避免用户在插件加载完成前立即按搜索时，query 被记录但没有实际搜索。本轮实现必须在 `searchAll()` 中保留 pending query，并在插件列表变为非空后补发搜索，避免同一类加载竞态在“快速输入并搜索”路径残留。
