...
| 触发                  | 必须连带改                                                                |
| --------------------- | ------------------------------------------------------------------------- |
| AGP 9.2.0             | Gradle wrapper 9.4.1（不能再用 milestone）；JDK toolchain ≥ 17（用 25）   |
...
1. **C1 toolchain**：Gradle wrapper 9.4.1 + AGP 9.2.0 + JDK toolchain 25 +
...
| R8 | Gradle 9.4.1 + JDK 25 toolchain 自动下载失败 / 公司网络限制                       | 低   | 高   | 升级前确认开发机已有 JDK 25；如失败回退用本地 `JAVA_HOME` 显式指向                          |
...
- 本机已装 JDK 25（`/usr/libexec/java_home -V`）；如未装，开启 Gradle auto-download
