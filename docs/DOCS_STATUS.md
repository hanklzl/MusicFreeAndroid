...
| [docs/superpowers/specs/2026-05-14-player-more-lyric-options-sheet-design.md](./superpowers/specs/2026-05-14-player-more-lyric-options-sheet-design.md) | 当前规范（播放页三点歌词操作浮窗 RN 对齐） | 是（作为实现计划输入） | 播放详情页封面页与歌词页三点按钮展示 RN `MusicItemLyricOptions` 风格底部浮窗 |
...
| [docs/superpowers/specs/2026-05-13-maestro-real-network-smoke-design.md](./superpowers/specs/2026-05-13-maestro-real-network-smoke-design.md) | 当前规范（Maestro 真实网络 smoke 验收） | 是（作为实现计划输入） | 使用真实网络链路编写 Maestro smoke、运行脚本与 logcat/结构化日志验收文档的设计 |
...
| [docs/superpowers/specs/2026-05-11-search-autofocus-once-design.md](./superpowers/specs/2026-05-11-search-autofocus-once-design.md) | 当前规范（搜索输入仅首次进入自动聚焦） | 是（作为实现计划输入） | 修正搜索页进入详情或播放器后返回时重复抢焦点并弹出输入法的问题 |
...
| [docs/superpowers/specs/2026-05-17-package-migration-backup-restore-design.md](./superpowers/specs/2026-05-17-package-migration-backup-restore-design.md) | 当前规范（包名迁移前备份与恢复） | 是（作为实现计划输入） | 旧包迁移版中备份与恢复入口、`.mfbackup` 迁移包格式、恢复登记与冷启动替换回滚设计 |
...
| [docs/superpowers/specs/2026-05-20-scene-pager-tabs-design.md](./superpowers/specs/2026-05-20-scene-pager-tabs-design.md) | 当前规范（搜索结果、推荐歌单与榜单 Scene Pager） | 是（作为实现计划输入） | 搜索结果一级媒体 Tab、二级插件 Tab、推荐歌单与榜单插件 Tab 升级为 ViewPager 式横向滑动，并为每个 Tab scene 保留独立状态 |
...
| [docs/superpowers/specs/2026-07-04-online-cache-management-list-design.md](./superpowers/specs/2026-07-04-online-cache-management-list-design.md) | 当前规范（在线播放缓存管理列表） | 是（作为实现计划输入） | 设置页“歌曲缓存管理”改为可搜索 / 筛选的歌曲缓存列表，按歌曲或音质清理在线播放缓存，不删除下载文件和本地音乐 |
